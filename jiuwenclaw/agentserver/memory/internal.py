...
def list_memory_files(
    workspace_dir: str, extra_paths: Optional[List[str]] = None
) -> List[str]:
...
def chunk_markdown(
    content: str, settings: Optional[Dict[str, Any]] = None
) -> List[MemoryChunk]:
    ...
    for i, line in enumerate(lines, 1):
        ...
        if current_tokens + line_tokens > target_tokens and current_chunk:
            chunk_text = "\n".join(current_chunk)
            chunks.append(
                MemoryChunk(text=chunk_text, startLine=start_line, endLine=i - 1)
            )
    ...
    if current_chunk:
        chunk_text = "\n".join(current_chunk)
        chunks.append(
            MemoryChunk(text=chunk_text, startLine=start_line, endLine=len(lines))
        )
...
def normalize_extra_memory_paths(
    paths: Optional[List[str]], workspace_dir: str
) -> List[str]:
