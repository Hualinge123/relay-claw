    ...
    def critique_descriptions(
        ...
        if len(examples) > 0 and prev_outputs is not None and len(prev_outputs) > 0:
            ...
            for output in prev_outputs[::-1][: self.config["num_feedback_steps"]][::-1]:
                ...
                for i, ((inst, fn_call, fn_output, ans), result) in enumerate(
                    zip(examples, output["results"]), 1
                ):
                    ...
                    for j, error in enumerate(result["errors"]):
                        user_prompt += (
                            f"({j}) function_call={error['function_name']},"
                ...
                user_prompt += (
                    f" score={output['score_avg']}%, stdev={output['score_std']}.\n"
                )
    ...
    def critique_negative_examples(
        ...
        if len(examples) > 0:
            ...
            for i, (inst, fn_call, fn_output, ans) in enumerate(examples, 1):
                ...
                user_prompt += (
                    f"  base on the original documentation: {json.dumps(fn_call)}.\n"
                )
                user_prompt += (
                    f"The runction output obtained is {fn_output}: fn_output. "
                )
    ...
    def critique_descriptions(
        ...
        if len(examples) > 0 and prev_outputs is not None and len(prev_outputs) > 0:
            ...
            if positive_examples:
                ...
                user_prompt += (
                    "The following tool descriptions achieved good performance:\n\n"
                )
            ...
            if negative_examples:
                ...
                user_prompt += (
                    "The following tool descriptions had poor performance:\n\n"
                )
    ...
    def critique_all_descriptions(
        ...
        if len(examples) > 0 and prev_outputs is not None and len(prev_outputs) > 0:
            ...
            if positive_examples:
                ...
                for i, (inst, fn_call, fn_output, ans) in enumerate(
                    positive_examples, 1
                ):
                    ...
                    if len(fn_output) > 256:
                        fn_output = fn_output[:256]
                        user_prompt += (
                            f"Example response of the function: {fn_output}, etc"
                        )
                    else:
                        user_prompt += f"Response of the function: {fn_output}"
            ...
            if negative_examples:
                ...
                for i, (inst, fn_call, fn_output, ans) in enumerate(
                    ...
                    user_prompt += (
                        f"function call system generated: {json.dumps(fn_call)}."
                    )
                    if len(fn_output) > 256:
                        fn_output = fn_output[:256]
...
"""
        ...
        for node_history in all_outputs:
            for step_output in node_history[::-1]:
                ...
                if "scores" in step_output.keys():
                    ...
                    if (
                        score >= 3.0 and isinstance(inst, str) and isinstance(ans, str)
                    ):  # td: SCORE THRESHOLD
                        selected_examples.append(
                            (inst.strip(), fn_call, fn_output, ans.strip())
                        )
                        break
                else:
                    pass
    ...
    def get_negative_examples(self, function_name):
        ...
        if os.path.exists(examples_path):
            ...
            logger.warning(
                f"NO NEGATIVE FILE FOUND at {examples_path}, FALLBACK TO LOAD GENERATED EXAMPLES"
            )
            examples_path = os.path.join(
                self.config["examples_dir"], f"{function_name}.json"
            )
            with open(examples_path, "r", encoding="utf-8") as f:
                all_outputs = json.load(f)
        if all_outputs is None:
        ...
        for node_history in all_outputs:
            for step_output in node_history[::-1]:
                ...
                if not all(
                    k in step_output
                    for k in ("instructions", "fn_call", "tool_results", "answers")
                ):
                    continue
                fn_call = step_output["fn_call"]
                fn_output = step_output["tool_results"]
    ...
    def get_examples(self, tool: Dict[str, Any]):
        ...
        logger.info(
            f"{len(examples)} Examples loaded for tool: {function_name}: {examples}"
        )
        return examples
