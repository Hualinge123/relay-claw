...
from openjiuwen.agent_evolving.optimizer.tool.utils.format import (
    format_prompt_llama,
    parse_json,
)
    ...
    def step(
        ...
        **kwargs,
        ...
        description = self.get_original_description(
            tool
        )  # get original descriptions for toolbench
        ...
        for _ in range(self.config["num_init_loop"]):
            fn_call = self.generate_api_call_from_description(
                tool_for_opt, num_gen=1, prev_output=prev_outputs
            )
        ...
        for n_refine in range(self.config["num_refine_steps"]):
            inst = self.generate_instruction_from_api_call(
                tool_for_opt, fn_call, tool_res, inst_output
            )
            ans = self.produce_answer_from_api_call(
                inst, json.dumps(tool_for_opt), tool_res
            )
            inst_eval = self.critique_instruction(
                tool_for_opt, inst, fn_call, tool_res, ans
            )
        ...
        if self.config["score_eval_weight"] > 0.0:
            ...
            if isinstance(insts[-1], str) and isinstance(answers[-1], str):
                ...
                eval_score = eval_res["score_avg"] / 100.0
            else:
                eval_score = 1.0
        else:
            logger.info("Eval step: hard coded eval_score as score_eval_weight=0 ")
            eval_score = 1.0
        final_score = scores[-1] + self.config["score_eval_weight"] * (1.0 - eval_score)
...
You must strictly follow the output format, including "name", "arguments", and parameters.\n"""
        ...
        if prev_output is not None and len(prev_output) > 0:
            ...
            user_prompt += (
                f"function {function_name}, which where then executed and critiqued:\n"
            )
            for i, output in enumerate(prev_output, 1):
                if "api_reflection" in output:
                    user_prompt += (
                        f'{i}. fn_call="{output["fn_call"]}" '
            ...
            user_prompt += (
                "You should improve your response based on these reflections.\n\n"
            )
        ...
        def verify_output(output):
            ...
            if "name" not in fn:
                raise ValueError(
                    'incorrect output format, "name" required for function'
                )
    ...
    def critique_api_call(
        self, tool: Dict[str, Any], fn_call: Dict[str, Any], fn_response: str
    ):
        function_name = tool["name"]
        doc_str = json.dumps(tool, ensure_ascii=False)
        user_prompt = f"""
...
You can begin your task now."""
        ...
        def verify_output(output):
            ...
            try:
                ...
                raise ValueError(
                    f'Invalid "err_code": {output_json.get("err_code")}'
                ) from exc
    ...
    def generate_instruction_from_api_call(
        ...
        if self.api_keys is not None and len(self.api_keys) > 0:
            user_prompt += (
                f"13. The instruction should include "
...
instructions for this function call, which were rated and analyzed:
        ...
        user_prompt += (
            "You must strictly follow the output format. Now you can begin your task."
        )
        prompt = format_prompt_llama(system_prompt="", user_prompt=user_prompt)
...
be a number between 1 and 3. You can begin your task now."""
        ...
        def verify_output(output):
            ...
            try:
                ...
                raise ValueError(
                    f'Invalid "score": {output_json.get("score")}'
                ) from exc
...
than 500 characters. You can now start your task."""
    ...
    def get_original_description(self, tool: Dict[str, Any]):
        ...
        description = (
            description[found + len(indicator) : -1] if found != -1 else description
        )
        return description
