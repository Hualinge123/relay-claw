...
class SimpleEval:
    ...
    def __init__(
        ...
        if abs(fn_call_weight + output_effectiveness_weight - 1.0) > 1e-6:
            raise ValueError(
                "fn_call_weight and output_effectiveness_weight must sum to 1.0"
            )
    ...
    def __call__(
        ...
        for run in range(runs):
            ...
            for i, (instruction, expected_fn_call, fn_output, answer) in enumerate(
                examples
            ):
            ...
            avg_fn_call_score = (
                total_fn_call_score / total_count if total_count > 0 else 0.0
            )
            avg_output_score = (
                total_output_score / total_count if total_count > 0 else 0.0
            )

            # Calculate weighted total score
            total_score = (
                self.fn_call_weight * avg_fn_call_score
    ...
    def _evaluate_single_example(self, example: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            generated_fn_call = self._generate_function_call(
                tool, description, instruction
            )

            # Step 2: Evaluate function call accuracy
            fn_call_score = self._evaluate_function_call_accuracy(
                generated_fn_call, expected_fn_call
            )
            ...
            if self.api_wrapper:
                try:
                    actual_output, status_code = self.api_wrapper(
                        tool, generated_fn_call
                    )
                    if status_code == 0:
            ...
            weighted_score = (
                self.fn_call_weight * fn_call_score
    ...
    def _generate_function_call(
        self, tool: Dict[str, Any], description: str, instruction: str
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            client = OpenAIModelClient(
                model_config=model_config, model_client_config=model_client
            )
    ...
    def _evaluate_function_call_accuracy(
        self, generated_fn_call: Dict[str, Any], expected_fn_call: Dict[str, Any]
    ) -> float:
        """
            ...
            if not exp_params and not gen_params:
                ...
                for key, expected_value in exp_params.items():
                    ...
                    if key in gen_params:
                        if self._compare_parameter_values(
                            gen_params[key], expected_value
                        ):
...
"""
    ...
    @staticmethod
    def _simple_output_comparison(execution_result: Any, expected_answer: str) -> float:
        """
