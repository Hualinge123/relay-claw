...
class ToolMgr:
    ...
    def get_mcp_tool(self, tool_name: str, server_id: str, session):
        ...
        if resource:
            tool_id = self.generate_mcp_tool_id(
                server_id, resource.config.server_name, tool_name
            )
    ...
    async def add_tool_server(
        ...
        try:
            ...
            results = await self._inner_refresh_mcp_tools(
                client, server_config, expiry_time
            )
            self._mcp_server_name_to_ids.setdefault(
                server_config.server_name, []
            ).append(server_config.server_id)
            return results
    ...
    async def remove_tool_server(
        self, server_id: str, ignore_not_exist: bool = True
    ) -> list[str]:
        ...
        try:
            ...
            ids = self._mcp_server_name_to_ids.get(
                mcp_server_resource.config.server_name
            )
    ...
    async def refresh_tool_server(
        ...
        if not force:
            if (
                ...
                and time.time() - mcp_resource.last_update_time
                >= mcp_resource.expiry_time
            ):
                need_refresh = True
