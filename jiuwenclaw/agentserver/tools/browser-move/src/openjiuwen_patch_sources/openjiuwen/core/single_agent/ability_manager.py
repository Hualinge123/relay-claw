...
class AbilityManager:
    ...
    ) -> Union[None, Ability, List[Ability]]:
    ...
    async def list_tool_info(
        ...
        for mcp_server_name, mcp_server in self._mcp_servers.items():
            mcp_server_id = getattr(mcp_server, "server_id", None) or getattr(
            ...
            if names is None:
                try:
                    mcp_tool_infos = await Runner.resource_mgr.get_mcp_tool_infos(
    ...
    async def execute(
        self, tool_call: Union[ToolCall, List[ToolCall]], session: Session
        ...
        for i, result in enumerate(results):
            if isinstance(result, Exception):
                ...
                tool_message = ToolMessage(
                    content=error_msg, tool_call_id=tool_calls[i].id
                )
    ...
    async def _execute_single_tool_call(
        ...
        if tool_name in self._tools:
            ...
            if agent:
                ...
                error_msg = f"Agent instance not found in resource_mgr: {agent_id}"
