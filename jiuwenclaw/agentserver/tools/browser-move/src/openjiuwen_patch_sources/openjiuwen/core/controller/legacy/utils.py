...
class MessageHandlerUtils:
    ...
    def format_llm_inputs(
        ...
        system_prompt = (
            PromptTemplate(content=config.prompt_template)
            .format(user_fields)
            .to_messages()
        )

        return MessageHandlerUtils.concat_system_prompt_with_chat_history(
            system_prompt, chat_history
        )

    @staticmethod
    def concat_system_prompt_with_chat_history(
        system_prompt: List[BaseMessage], chat_history: List[BaseMessage]
    ) -> List[BaseMessage]:
        result_messages = []
    ...
    def create_tasks_from_tool_calls(
        tool_calls: List[ToolCall], config: AgentConfig
    ) -> List[Task]:
        if not tool_calls:
            return []
                ...
                if workflow.name == tool_name:
                    ...
                    try:
                        ...
                        if UserConfig.is_sensitive():
                            logger.error(
                                "LLM Agent parse tool call workflow's arguments error"
                            )
                            raise build_error(
                                StatusCode.AGENT_CONTROLLER_TOOL_EXECUTION_PROCESS_ERROR,
                                error_msg="LLM-generated workflow arguments are invalid",
                            ) from e
                        else:
                            logger.error(
                                f"LLM Agent parse tool call workflow({tool_name})'s arguments error: "
                                f"{tool_call.arguments}"
                            )
                            raise build_error(
                                StatusCode.AGENT_CONTROLLER_TOOL_EXECUTION_PROCESS_ERROR,
                                error_msg=f"LLM-generated workflow ({tool_name}) arguments "
                                f"are invalid: {tool_call.arguments}",
                            ) from e
            ...
            for plugin in config.plugins:
                if plugin.name == tool_name:
                    ...
                    try:
                        ...
                        if UserConfig.is_sensitive():
                            logger.error(
                                "LLM Agent parse tool call plugin's arguments error"
                            )
                            raise build_error(
                                StatusCode.AGENT_CONTROLLER_TOOL_EXECUTION_PROCESS_ERROR,
                                error_msg="LLM-generated plugin arguments are invalid",
                            ) from e
                        else:
                            logger.error(
                                f"LLM Agent parse tool call plugin({tool_name})'s arguments error: "
                                f"{tool_call.arguments}"
                            )
                            raise build_error(
                                StatusCode.AGENT_CONTROLLER_TOOL_EXECUTION_PROCESS_ERROR,
                    ...
                    result.append(
                        Task(
                            task_id=tool_call.id,
                            input=TaskInput(target_name=tool_name, arguments=arguments),
                            task_type=task_type,
                        )
                    )
                    break
    ...
    def determine_task_type(tool_name: str, config: AgentConfig) -> TaskType:
        ...
        raise build_error(
            StatusCode.AGENT_TOOL_NOT_FOUND,
            error_msg=f"not find tool call type: {tool_name}",
        )
    ...
    def is_interaction_result(exec_result: Any) -> bool:
        return (
            ...
            and isinstance(exec_result.get("value"), list)
        )

    @staticmethod
    def create_interrupt_result(e, tool_name: str) -> Dict[str, Any]:
        return {"error": True, "value": e.message, "tool_name": tool_name}
    ...
    def should_add_user_message(
        query: str, context_engine: ContextEngine, session: Session
    ) -> bool:
    ...
    async def add_user_message(
        query: Any, context_engine: ContextEngine, session: Session
    ):
    ...
    async def add_ai_message(
        ai_message: AssistantMessage, context_engine: ContextEngine, session: Session
    ):
        if ai_message:
            agent_context = context_engine.get_context(
                session_id=session.get_session_id()
    ...
    async def add_tool_result(
        event: Event, context_engine: ContextEngine, session: Session
    ):
        if event:
            agent_context = context_engine.get_context(
                session_id=session.get_session_id()
    ...
    def get_chat_history(
        context_engine: ContextEngine, session: Session, config: AgentConfig
    ) -> List[BaseMessage]:
        agent_context = context_engine.get_context(session_id=session.get_session_id())
    ...
    @staticmethod
    async def add_workflow_message_to_chat_history(
        message: BaseMessage,
        workflow_id: str,
        ...
        workflow_context = context_engine.get_context(
            context_id=workflow_id, session_id=session.get_session_id()
        )
...
class ReasonerUtils:
    @staticmethod
    def get_chat_history(
        context_engine: ContextEngine, session: Session, chat_history_max_turn: int
    ) -> List[BaseMessage]:
