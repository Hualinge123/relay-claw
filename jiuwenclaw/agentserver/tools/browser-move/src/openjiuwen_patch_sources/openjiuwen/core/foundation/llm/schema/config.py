...
class ModelClientConfig(BaseModel):
    ...
    model_config = {
        "extra": "allow"
    }  # Allow extra fields like user_id for ICBC integrations.
...
class ModelRequestConfig(BaseModel):
    ...
    model_name: str = Field(
        default="", alias="model", description="Model name, e.g. gpt-4"
    )
    temperature: float = Field(
        default=0.95,
        description="Temperature parameter, controlling the randomness of outputs",
    )
