...
class ReActAgentConfig(BaseModel):
    ...
    prompt_template: List[Dict] = Field(
        default_factory=list, description="Prompt template list"
    )
    ...
    model_client_config: Optional[ModelClientConfig] = Field(
        default=None, description="Model client configuration"
    )
    model_config_obj: Optional[ModelRequestConfig] = Field(
        default=None, description="Model request configuration"
    )
    ...
    def configure_context_processors(
        self, processors: List[Tuple[str, BaseModel]]
    ) -> "ReActAgentConfig":
...
class ReActAgent(BaseAgent):
    ...
    async def _call_llm(
        self, messages: List, tools: Optional[List[ToolInfo]] = None
    ) -> AssistantMessage:
    ...
    async def invoke(
        ...
        if (
            ...
            and self._skill_util is not None
            and self._skill_util.has_skill()
        ):
        ...
        for iteration in range(self._config.max_iterations):
            ...
            ai_message = await self._call_llm(
                context_window.get_messages(), context_window.get_tools() or None
            )
