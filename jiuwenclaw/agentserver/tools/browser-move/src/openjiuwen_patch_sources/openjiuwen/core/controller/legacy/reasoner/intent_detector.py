...
class IntentDetector:
    ...
    def _generate_tasks_from_intent(self, intent_id: str, event: Event) -> List[Task]:
        """
        Create task objects
        ...
        if not workflows:
            task_input = TaskInput(
                target_id=intent_id, target_name=intent_id, arguments=event.content
            )
            task = Task(
        ...
        for workflow in workflows:
            if workflow.id == intent_id:
                ...
                tasks.append(task)
                logger.info(
                    f"[%s] success to create task for intent: %s", session_id, intent_id
                )
                break
        return tasks
    ...
    def _parse_intent_from_output(self, llm_output: str) -> str:
        ...
        try:
            ...
            if detected_class_number <= 0 or detected_class_number > len(
                self.intent_config.category_list
            ):
                # Unknown intent
                logger.warning("get unknown class")
            else:
                detected_intent_name = self.intent_config.category_list[
                    detected_class_number - 1
                ]
    ...
    async def _invoke_llm_get_output(
        ...
        try:
            model = await ReasonerUtils.get_model(self.agent_config.model)
            llm_output = await model.invoke(
                llm_inputs, model=self.agent_config.model.model_info.model_name
            )
    ...
    def _prepare_detection_input(self, event: Event) -> str:
        ...
        category_list = "分类0：意图不明\n" + "\n".join(
            f"分类{i+1}：{c}" for i, c in enumerate(self.intent_config.category_list)
        )
        current_inputs = {}
        ...
        llm_inputs = self.intent_config.intent_detection_template.format(
            current_inputs
        ).to_messages()
        return llm_inputs
