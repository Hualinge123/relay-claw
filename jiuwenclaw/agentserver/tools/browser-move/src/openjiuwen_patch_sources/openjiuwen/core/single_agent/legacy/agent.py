...
class WorkflowFactory:
    ...
    def __init__(
        ...
        self._workflow_card = WorkflowCard(
            id=workflow_id,
            name=workflow_name,
...
class BaseAgent(ABC):
    ...
    def _create_context_engine(self) -> ContextEngine:
        ...
        if hasattr(self.agent_config, "constrain") and hasattr(
            self.agent_config.constrain, "reserved_max_chat_rounds"
        ):
    ...
    def add_workflows(
        self, workflows: List[Union[Workflow, Callable[[], Workflow]]]
    ) -> None:
        ...
        for item in workflows:
            ...
            if isinstance(item, WorkflowFactory):
                ...
                workflow_card = WorkflowCard(
                    ...
                    input_params=getattr(item, "input_params", None)
            ...
            try:
                logger.info(
                    f"Adding workflow {'provider' if is_provider else 'instance'} "
                ...
                Runner.resource_mgr.add_workflow(
                    card=workflow_card_copy,
                    workflow=to_register,
    ...
    def remove_workflows(self, workflows: List[Tuple[str, str]]) -> None:
        ...
        """
        logger.info(
            f"BaseAgent.remove_workflows called with {len(workflows)} workflows"
        )
        ...
        for workflow_id, workflow_version in workflows:
            ...
            try:
                ...
                logger.info(
                    f"Successfully removed workflow {workflow_key} from global resource_mgr"
                )
            except Exception as e:
    ...
    async def invoke(self, inputs: Dict, session: Session = None) -> Dict:
        ...
        if session is None:
            ...
            agent_session = create_agent_session(
                session_id=session_id, card=AgentCard(id=self.agent_config.id)
            )
    ...
    async def stream(self, inputs: Dict, session: Session = None) -> AsyncIterator[Any]:
        ...
        async def stream_process():
            try:
                ...
                if isinstance(res, list) and isinstance(
