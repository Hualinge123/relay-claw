...
class IntentDetectionDefaultConfig:
    ...
    intent_detection_template: PromptTemplate = field(
        default_factory=get_default_template
    )
...
class IntentDetectionExecutable(ComponentExecutable):
    ...
    def _get_category_info(self):
        return "\n".join(
            ...
            for cid, cname in zip(
                self._default_config.category_list, self._config.category_name_list
            )
        )
    ...
    def _pre_process(self, inputs: dict):
        ...
        final_prompts = self._default_config.intent_detection_template.format(
            inputs
        ).to_messages()
    ...
    def _parse_detection_result(self, llm_output):
        ...
        return IntentDetectionOutput(
            classification_id=intent_id_and_name.get(CLASSIFICATION_ID, -1),
            reason=reason,
    ...
    async def _invoke_llm_and_get_result(self, current_inputs):
        ...
        llm_inputs = self._default_config.intent_detection_template.format(
            current_inputs
        ).to_messages()
        workflow_logger.info(
    ...
    def _post_process_intent_detection(self, result):
        ...
        try:
            ...
            return self._default_config.default_class, JSON_PARSE_FAIL_REASON.format(
                result=result
            )
        ...
        if not parsed_dict.get(CLASS):
            return self._default_config.default_class, CLASS_KEY_MISSING_REASON.format(
                result=parsed_dict
            )
        ...
        if not parsed_dict.get(CLASS) in self._default_config.category_list:
            ...
            parsed_dict.update(
                {CLASS: self._default_config.default_class, REASON: reason}
            )
    ...
    def _append_default_category(self):
        self._default_config.category_list = [
            self._default_config.default_class
        ] + self._default_config.category_list
        self._config.category_name_list = [
            CLASSIFICATION_DEFAULT_NAME
        ] + self._config.category_name_list

    def _get_intent_id_and_name(self, intent_class):
        intent_res = {
            CLASSIFICATION_ID: CLASSIFICATION_DEFAULT_ID,
            CLASSIFICATION_NAME: CLASSIFICATION_DEFAULT_NAME,
        }
        idx = next(
            (
                i
                for i, category in enumerate(self._default_config.category_list)
        ...
        if idx > -1:
            intent_res = {
                ...
                CLASSIFICATION_NAME: self._config.category_name_list[idx],
            }
        return intent_res

    def _init_default_config_category_list(
        self, component_config: IntentDetectionCompConfig
    ):
