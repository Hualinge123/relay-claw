...
async def init_memory_manager_async(
    workspace_dir: str = ".", agent_id: str = "default"
    ...
    try:
        _global_manager = await MemoryIndexManager.get(
            agent_id=agent_id, workspace_dir=workspace_dir, settings=settings
        )
...
async def memory_get(
    path: str, from_line: Optional[int] = None, lines: Optional[int] = None
) -> Dict[str, Any]:
    ...
    try:
        result = await _global_manager.read_file(
            rel_path=path, from_line=from_line, lines=lines
        )
        return {**result, "disabled": False}

    except Exception as e:
        logger.error(f"Memory get failed: {e}")
        return {"path": path, "text": "", "disabled": True, "error": str(e)}
...
@tool
async def edit_memory(path: str, oldText: str, newText: str) -> Dict[str, Any]:
